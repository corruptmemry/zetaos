	...
	for(uint32_t bus = 0; bus < 256; bus++)
    {
        for(uint32_t slot = 0; slot < 32; slot++)
        {
            for(uint32_t function = 0; function < 8; function++)
            {
                    ...
                    uint16_t dclass = getClassId(bus, slot, function);
                    Print(0xFFFFFF, "\npci_probe : found a ");
                    Print(0xFFFFFF, DeviceClasses[dclass]);
                    Print(0xFFFFFF, " with vendor ");
                    Print(0xFFFFFF, GetVendorName(vendor));
                    pci_device *pdev = (pci_device *)malloc(sizeof(pci_device));
                    pdev->vendor = vendor;
                    pdev->device = device;
                    pdev->func = function;
                    pdev->vendorname = GetVendorName(vendor);
                    pdev->deviceclass = DeviceClasses[dclass];
                    pdev->driver = 0;
                    add_pci_device(pdev);
            }
        }
...
void pci_proc_dump(uint8_t *buffer)
{
	for(int i = 0; i < devs; i++)
	{
		...
		if(pci_dev->driver)
			Print(0xFFFFFF, pci_dev->driver->name);
		else
			Print(0xFFFFFF, pci_dev->vendor);
	}
}