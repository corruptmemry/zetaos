...
void rtc_read_datetime() {
    ...
    if (!(registerB & 0x04)) {
        ...
        current_datetime.hour = (( (current_datetime.hour & 0x0F) + (((current_datetime.hour & 0x70) / 16) * 10) ) | (current_datetime.hour & 0x80)) + 7; // MSK time
...
char get_time(datetime_t * dt){
    ...
    strcat(ret, to_string64((uint64_t)current_datetime.hour));
    strcat(ret, ":");
    strcat(ret, to_string64((uint64_t)current_datetime.minute));
    strcat(ret, ":");
    strcat(ret, to_string64((uint64_t)current_datetime.second));
    return ret;
}
